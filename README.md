...
Each component is built and individually checked; the **end-to-end live
integration is still being finished** across the agent branches (see
[`docs/INTEGRATION_CHECKLIST.md`](docs/INTEGRATION_CHECKLIST.md)). Demo today in
`mockAll`.

- ✅ **Backend** — typecheck clean, **49 unit tests passing**, smoke script green; all contract functions implemented.
- ✅ **iOS (mock mode)** — runs in `mockAll` (camera, Brain, voice/chips, profiles, drafts). Pure logic + non-UI layers are compile-checked; the **full Xcode build is verified by the iOS owner** (agent environments have no Xcode).
- 🟡 **CV service** — code present and `py_compile`-clean; produces 512-d L2-normalized embeddings **when installed** (Python 3.10–3.11). A full `/embed` run is owner-verified.
- 🔌 **Live integration (in progress)** — `ConvexBackend.swift` still delegates to the offline mock, so the real iOS → backend → CV path isn't wired on `main` yet. Remaining: the backend HTTP bridge, real face enrollment, wiring the iOS live client/overlay, and on-device threshold tuning.
